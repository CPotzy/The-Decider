...
class PressureTierTest {
    ...
    @Test fun `any positive pressure daily is OVERDUE`() =
        assertEquals(PressureTier.OVERDUE, PressureTier.forPressure(0.1, Cadence.DAILY))

    @Test fun `pressure 1 daily is OVERDUE`() =
        assertEquals(PressureTier.OVERDUE, PressureTier.forPressure(1.0, Cadence.DAILY))
